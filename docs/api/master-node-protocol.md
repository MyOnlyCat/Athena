...
`success` 或 `failure` 结果、来源 IP、失败机器码和 UTC RFC 3339 时间。页面按浏览器
时区显示并标注时区。

记录范围是管理员登录、管理员账号创建/启停/密码重置、注册申请批准/拒绝/恢复，以及
接入节点管理信息修改、启停和 Token 更换。未认证登录失败的操作者为空，提交用户名
仅作为目标；无论账号是否存在，响应均使用相同的认证失败契约。

审计记录没有通用请求详情字段，也不会读取或保存请求正文。密码、JWT、Node Token
明文/密文/指纹、注册原始正文和签名均不得进入审计。Node 发起的注册提交与状态查询、
